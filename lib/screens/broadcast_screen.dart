...
import 'package:streamingapp/config/keys.dart';
import 'package:streamingapp/providers/user_provider.dart';
import 'package:streamingapp/resources/resources.dart';
...
class _BroadcastScreenState extends State<BroadcastScreen> {
  ...
  Future<void> getToken() async {
    ...
    if (res.statusCode == 200) {
      ...
      dev.log('Failed to fetch the token');
    }
  }
  ...
  void _addListeners() {
    _engine.setEventHandler(
      ...
      dev.log('joinChannelSuccess $channel $uid $elapsed');
    }, userJoined: (uid, elapsed) {
      dev.log('userJoined $uid $elapsed');
      setState(() {
        remoteUid.add(uid);
      });
    }, userOffline: (uid, reason) {
      dev.log('userOffline $uid $reason');
      setState(() {
        remoteUid.removeWhere((element) => element == uid);
      });
    }, leaveChannel: (stats) {
      dev.log('leaveChannel $stats');
      setState(() {
        remoteUid.clear();
      });
  ...
  _startScreenShare() async {
    ...
    if (!kIsWeb &&
      ...
      if (windows.isNotEmpty) {
        ...
        dev.log('Screensharing window with index $index');
  ...
  _leaveChannel() async {
    try {
      ...
      if ('${usersProvider.uid}${usersProvider.username}' == widget.channelId) {
